...
client.file(fileId)
  ...
  .then(components => {
    ...
    return queueTasks(Object.values(components).map(component => () => {
      const fileName = `${component.name}.${options.format}`.replace(/\//g, '-'); // Replace "/" with "-"
      return got.get(component.image, {
        ...
        .then(response => {
          return ensureDir(join(options.outputDir, options.format))
            .then(() => writeFile(join(options.outputDir, options.format, fileName), response.body, (options.format === 'svg' ? 'utf8' : 'binary')));
        });
    }));
  })
  .catch(error => {
